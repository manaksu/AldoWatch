...
/* ── Aldo Watchface ─────────────────────────────────────────────────────
   Creamy white background, charcoal text, Aldo the Apache font.
   Layout:
     Time  (48px): x=12, y=62  — left aligned
     AM/PM (18px): x=110, y=66 — right of time
     Date  (14px): x=22, y=100 — 10px right of time start, 4px below
   ──────────────────────────────────────────────────────────────────── */
...
static void window_load(Window *w) {
  ...
  s_font_ampm = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ALDO_18));
  s_font_date = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ALDO_14));
  ...
  /* AM/PM */
  s_ampm_layer = text_layer_create(GRect(AMPM_X, AMPM_Y, 30, 22));
  ...
  /* Date */
  s_date_layer = text_layer_create(GRect(DATE_X, DATE_Y, 122, 18));
